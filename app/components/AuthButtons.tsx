...
export const AuthButtonOutlined = ({ children, slug }: BtnProps) => {
    return (
        <Link href={`/${slug}`}
            className='inline-flex items-center justify-center h-10 min-h-[40px] rounded-full w-full bg-white border border-[#0082fa] py-[10px] px-4 text-[#0082fa] text-center select-none hover:bg-[#0082fa] hover:text-white active:text-white active:bg-[#00407a] transition-colors duration-200'
        >
            {children}
        </Link>
    )
