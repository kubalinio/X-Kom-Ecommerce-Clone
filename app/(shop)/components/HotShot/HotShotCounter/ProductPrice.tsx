...
export const ProductPrice = ({ finished, price, oldPrice, promotionGainTextLines }: Props) => {
    ...
    return (
        <div className='mt-2 mb-3 text-sm md:mt-8 lg:mt-2 font-lato'>
            <p className={`block text-center text-4xl/10 text-[#4d4d4d] font-bold lg:mb-4 ${finished ? 'text-[#4d4d4d]' : 'text-[#fa0064]'}`}>
                {price.toString().replace('.', ',')} zł
            </p>

            <div className="hidden md:block lg:hidden">
                <p className="text-lg/6 mb-2 text-[#fa0064]">{promotionGainTextLines.title} {promotionGainTextLines.value}</p>
            </div>
